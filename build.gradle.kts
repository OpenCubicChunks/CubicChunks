import io.github.opencubicchunks.gradle.Utils
...
minecraft {
    ...
    fun setupConfig(conf: RunConfig) {
        conf.apply {
            ...
            mods.create("cubicchunks") {
                source(java.sourceSets["main"])
                source(java.sourceSets["api"])
            }
        }
    }
...
tasks.create("generateTestRunData").doLast {
    val javaConv = Utils.getJavaPluginConvention(project)
    val testSrcSet: SourceSet = javaConv.sourceSets["test"]
    val resourcesSet = testSrcSet.resources.srcDirs
    val resources = resourcesSet.iterator().next().canonicalFile.toPath()
    logger.lifecycle("Generating data for test runner...")
    val environment = resources.resolve("env.txt")

    `java.nio.file`.Files.newBufferedWriter(environment).use {
        val properties = minecraft.runs["data"].environment.toProperties()
        var modClasses = properties.getProperty("MOD_CLASSES")
        for (srcDir in testSrcSet.resources.srcDirs) {
            modClasses += ":testrunner%%" + srcDir.absolutePath
        }
        properties.setProperty("MOD_CLASSES", modClasses)
        properties.store(it, "autogenerated by gradle task generateTestRunData")
    }
}
tasks["test"].dependsOn("generateTestRunData");
