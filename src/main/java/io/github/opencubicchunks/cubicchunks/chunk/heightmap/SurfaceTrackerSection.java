...
//		super((ChunkAccess) cube, types);
	...
	@Nullable
	private SurfaceTrackerSection loadNode(int newScaledY, int scale, IBigCube newCube, boolean create) {
		// TODO: loading from disk
		if (!create) {
			return null;
		}
		if (scale == 0) {
			return new SurfaceTrackerSection(scale, newScaledY, this, newCube, HEIGHTMAP_TYPES[this.heightmapType]);
		}
		return new SurfaceTrackerSection(scale, newScaledY, this, HEIGHTMAP_TYPES[this.heightmapType]);
	}
