...
public class PacketCubes {
    ...
    public PacketCubes(List<ICube> cubes) {
        this.positions = cubes.stream().map(ICube::getPos).toArray(SectionPos[]::new);
    ...
    private static void fillDataBuffer(PacketBuffer buf, List<ICube> cubes, BitSet existingChunks) {
    ...
    private static int calculateDataSize(List<ICube> cubes) {
        return cubes.stream().map(ICube::getStorage)
